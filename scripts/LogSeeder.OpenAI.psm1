...
function Get-ScenarioMenuItems {
    ...
    $items += [pscustomobject]@{
        ...
        Description = "Use AI to turn a short idea into next steps."
        Value = "__other__"
    }
    return $items
}
...
function New-RuntimeScenario {
    ...
    foreach ($phase in @($Scenario.timeline)) {
        ...
        $phaseCountsByCategory[$category] += $originalCount
    ...
    foreach ($tableProp in $Scenario.tables.PSObject.Properties) {
        ...
        if ($phaseCountsByCategory.ContainsKey($category) -and $originalRows -lt $phaseCountsByCategory[$category]) {
            $originalRows = $phaseCountsByCategory[$category]
        }

        if ($runtimeTables.Contains($tableName)) {
            $runtimeTables[$tableName].rowCount = [int]$runtimeTables[$tableName].rowCount + $originalRows
        } else {
            $runtimeTables[$tableName] = [ordered]@{
                schema = $mapEntry.Schema
                rowCount = $originalRows
            }
        }
    }
    ...
    $runId = (($Scenario.name -replace '[^A-Za-z0-9]+', '-').Trim('-') + "-" + ($generatedAtUtc -replace '[^0-9]', ''))
    $runtime = [ordered]@{
        name = $runtimeName
        description = "$($Scenario.description) Runtime generated by Start-LogSeederOpenAI.ps1 using the original upstream scenario counts."
...
function Invoke-PrebuiltScenarioWorkflow {
    ...
    Write-Host ""
    Write-Host "Using original scenario size from the repository." -ForegroundColor Cyan
    ...
    if (($mode.Value -eq "deployIngest" -or $previewChoice) -and -not $createDetection) {
        ...
        Write-Host "The rule will create a Sentinel incident when the scenario logs match the detection logic." -ForegroundColor Gray
...
function Invoke-OtherRequestWorkflow {
    if (-not (Test-OpenAIConfigured)) {
        Write-Host ""
        Write-Host "The 'other' path uses AI to interpret your request." -ForegroundColor Yellow
        Write-Host 'Set OpenAI or Azure OpenAI environment variables first, or use one of the menu options above.' -ForegroundColor Gray
        return
    }
